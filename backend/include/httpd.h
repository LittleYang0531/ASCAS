        ...
        for (int i = 0; i < app.route.size(); i++) {
            if (app.matchPath(app.route[i], rlpath)) {
                ...
                try {
                    ...
                    writeLog(LOG_LEVEL_ERROR, "Terminate called after throwing an instance. what(): %s", e.what());
                    return;
                } catch(...) {
                    writeLog(LOG_LEVEL_ERROR, "Terminate called after throwing an instance.");
                    return;
                }
...
void StaticFileDownloader(client_conn &conn, http_request request, std::string file) {
    /** 打开文件 */
    std::ifstream fin(file, std::ios::binary);
    if (!fin.is_open()) exitRequest(conn);

    /** 构造基础响应头 */
    argvar response = __default_response;
    response = merge(response, mime(file.substr(file.rfind("."))));
...
void StaticFileDownloader(client_conn &conn, http_request request, std::string content, std::string mime) {
    /** 构造基础响应头 */
    argvar response = __default_response;
    response["Content-Type"] = mime;

    const __int64_t len = 1024 * 1024;
    __int64_t filesize = content.size();
    __int64_t st = 0, en = filesize - 1;
    ...
    memset(c, '\0', len);
    while (output_len) {
        memcpy(c, content.c_str(), std::min(len, output_len));
        int s = send(conn, c, std::min(len, output_len));
        if (s == -1) exitRequest(conn);
        output_len -= std::min(len, output_len);
    }
    exitRequest(conn);
