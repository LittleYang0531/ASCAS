...
struct Connection {
    ...
    bool send(std::string msg) {
        ...
        while(true) {
            ...
            if (s == -1) {
                writeLog(LOG_LEVEL_WARNING, "Failed to send message! Errno: %d", errno);
                return false;
            }
            send += s;
    ...
    std::string recv() {
        ...
        while(true) {
            ...
            if (s == -1) {
                writeLog(LOG_LEVEL_WARNING, "Failed to recv message! Error: %d", errno);
                return errorKey;
            }
            recv += s;
...
struct Server {
    ...
    Server(std::string host, int port): isUnix(false) {
        ...
        if (sock < 0) {
            writeLog(LOG_LEVEL_WARNING, "Failed to create socket! Errno: %d", errno);
            return;
        }
        ...
        if (ret < 0) {
            writeLog(LOG_LEVEL_WARNING, "Failed to bind address! Errno: %d", errno);
            return;
        }
        ...
        ret = listen(sock,1);
        if (ret < 0) {
            writeLog(LOG_LEVEL_WARNING, "Failed to set listening status! Errno: %d", errno);
            return;
        }
    }
    ...
    Connection accept() {
        ...
        if (conn < 0) {
            writeLog(LOG_LEVEL_WARNING, "Failed to accept client! Errno: %d", errno);
            return Connection({});
        }
        return Connection({
            .isUnix = isUnix,
...
struct Client {
    ...
    Connection connect() {
        ...
        if (conn < 0) {
            writeLog(LOG_LEVEL_WARNING, "Failed to connect to server! Errno: %d", errno);
            return Connection({});
        }
        return Connection({
            .isUnix = isUnix,
